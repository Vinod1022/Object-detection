...
# ---------------------------
# Custom class mapping (IMPORTANT)
# ---------------------------
CLASS_MAP = {
    0: "with_helmet",
    1: "without_helmet"
...
if model_source == "Use my best.pt (default)":
    model_path = "best.pt"     # Your custom model must be here
else:
...
if st.button("Run Detection"):
    ...
    if df.empty:
        ...
        if "with_helmet" in class_counts:
            st.success(f"🟢 Helmet Detected: {class_counts['with_helmet']}")

        if "without_helmet" in class_counts:
            st.error(f"🔴 No Helmet: {class_counts['without_helmet']}")
